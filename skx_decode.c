...
/* ---- decode controls / pre-scan ---- */

static bool g_verbose = false;
/* usable only if at least one RIR_VALID=1 exists for that channel */
static bool g_chan_usable[2][2][3];

static void skx_scan_usable_channels(SkxSocket socks[2])
{
        ...
        for (int imc = 0; imc < 2; imc++) {
            for (int ch = 0; ch < 3; ch++) {
                ...
                for (int i = 0; i < SKX_MAX_RIR; i++) {
                    uint32_t way = 0;
                    if (pci_read_u32(s->imc[imc].chan[ch].cdev, 0x108 + 4*i, &way) != 0) {
                        ok = false;
                        break;
                    }
...
/* TAD decode + de-interleave (so chan_addr is channel-local and matches RIR limits). */
static bool skx_tad_decode(SkxSocket socks[2], DecodedAddr *r)
{
    ...
    for (int i = 0; i < SKX_MAX_TAD; i++) {
        ...
        /* reject weirdness we do not model (keeps failure rate low) */
        if (!(r->sktways == 1 || r->sktways == 2 || r->sktways == 4)) return false;
        if (!(r->chanways >= 1 && r->chanways <= 4)) return false;

        uint64_t tmp = addr - base;

        if (r->sktways > 1)  tmp /= (uint64_t)r->sktways;
        if (r->chanways > 1) tmp /= (uint64_t)r->chanways;
...
/* RIR decode with:
 *  - rule selection using channel-local addr ranges
 *  - ILV lookup using probed selector shift (6..16) to find a populated entry */
static bool skx_rir_decode(SkxSocket socks[2], DecodedAddr *r)
{
    ...
    for (int i = 0; i < SKX_MAX_RIR; i++) {
        if (pci_read_u32(s->imc[r->imc].chan[r->channel].cdev, 0x108 + 4*i, &way) != 0) {
            if (g_verbose) fprintf(stderr, "RIR_FAIL: read wayness i=%d\n", i);
            return false;
        }
        ...
        if (SKX_RIR_VALID(way)) {
            if (addr >= prev_limit && addr <= limit) {
                ...
                rule = i;
                if (g_verbose) {
                    fprintf(stderr, "RIR: selected rule=%d (addr in [0x%llx..0x%llx]) ways=%d\n",
                            rule,
                            (unsigned long long)prev_limit,
                            (unsigned long long)limit,
                            r->chanways);
    ...
    if (rule < 0) {
        if (g_verbose) fprintf(stderr, "RIR_FAIL: no matching rule for chan_addr=%#" PRIx64 "\n", addr);
        return false;
    }
    ...
    for (int sh = min_shift; sh <= max_shift; sh++) {
        ...
        for (int w = 0; w < 8; w++) {
            ...
            if (pci_read_u32(s->imc[r->imc].chan[r->channel].cdev,
                ...
                if (g_verbose) fprintf(stderr, "RIR_FAIL: read ILV sh=%d idx=%d w=%d\n", sh, idx, w);
                return false;
            }

            if (g_verbose) {
                fprintf(stderr, "RIR:   ilv[sh=%d idx=%d w=%d] = 0x%08x\n", sh, idx, w, ilv);
            }
    ...
    if (g_verbose) fprintf(stderr, "RIR_FAIL: no populated ILV entry found\n");
    return false;

ilv_found:
    if (g_verbose) {
        fprintf(stderr, "RIR: chose ILV via shift=%d idx=%d w=%d ilv=0x%08x\n",
...
static bool skx_decode(SkxSocket socks[2], DecodedAddr *r)
{
    ...
    /* Option B: discard addresses routed to channels that have no valid RIR rules */
    if (!g_chan_usable[r->socket][r->imc][r->channel]) {
        if (g_verbose) fprintf(stderr, "FAIL: channel unusable (no RIR_VALID) sock=%d imc=%d chan=%d\n",
...
int main(int argc, char **argv)
{
    ...
    /* Mark which channels are decodable at RIR level. */
    skx_scan_usable_channels(socks);

    /* tohm/tolm: keep conservative */
    g_tolm = 0;
    g_tohm = ~0ULL;
