...
static void usage(const char *argv0) {
    fprintf(stderr,
        ...
        "            [--timing-check BANKID] [--timing-iters N] [--timing-trials N]\n"
        "\n"
        ...
        "\n"
        "  --timing-check   run a simple rdtsc timing sanity-check for BANKID\n"
        "  --timing-iters   iterations per measurement (default 200000; clamped)\n"
        "  --timing-trials  number of pairs to test (default 8; clamped)\n",
        argv0);
    exit(1);
}
...
static int pick_two_in_same_bank_same_context_diff_row(struct vec *bank,
                                                       struct hit **out_a,
                                                       struct hit **out_b)
{
    // Pick two hits in the same bank with identical (socket,imc,channel,dimm,rank)
    // but different row. This tends to create row-buffer thrashing when alternated.
    for (size_t i = 0; i < bank->n; i++) {
        struct hit *a = &bank->v[i];
        for (size_t j = i + 1; j < bank->n; j++) {
            struct hit *b = &bank->v[j];
            if (a->socket != b->socket) continue;
            if (a->imc != b->imc) continue;
            if (a->channel != b->channel) continue;
            if (a->dimm != b->dimm) continue;
...
static int pick_cross_bank_same_context(struct vec banks[16], int bank_id,
                                        struct hit *ref,
                                        struct hit **out_other)
{
    // Find a hit from a different bank_id but with same (socket,imc,channel,dimm,rank).
    for (int ob = 0; ob < 16; ob++) {
        if (ob == bank_id) continue;
        for (size_t j = 0; j < banks[ob].n; j++) {
            struct hit *h = &banks[ob].v[j];
            if (h->socket != ref->socket) continue;
            if (h->imc != ref->imc) continue;
            if (h->channel != ref->channel) continue;
            if (h->dimm != ref->dimm) continue;
            if (h->rank != ref->rank) continue;
...
static void run_timing_check(struct vec banks[16], int bank_id,
                             uint64_t iters, int trials)
{
    ...
    printf("\n=== TIMING SANITY-CHECK (bank_id=%d, iters=%" PRIu64 ", trials=%d) ===\n",
           bank_id, iters, trials);
    printf("This compares: (1) two addresses in the same bank_id but different rows\n");
    printf("vs (2) addresses in different bank_id values, trying to keep\n");
    printf("socket/imc/channel/dimm/rank constant.\n\n");

    uint64_t same_sum = 0, diff_sum = 0;
    int same_ok = 0, diff_ok = 0;

    for (int t = 0; t < trials; t++) {
        struct hit *a = NULL, *b = NULL;
        if (pick_two_in_same_bank_same_context_diff_row(&banks[bank_id], &a, &b) != 0) {
            warnx("timing-check: could not find same-bank same-context diff-row pair for bank_id=%d", bank_id);
            break;
        }

        volatile uint8_t *va = (volatile uint8_t *)(uintptr_t)a->va;
        volatile uint8_t *vb = (volatile uint8_t *)(uintptr_t)b->va;
        uint64_t cyc_same = measure_pair_cycles(va, vb, iters);
        same_sum += cyc_same;
        same_ok++;

        struct hit *c = NULL;
        if (pick_cross_bank_same_context(banks, bank_id, a, &c) == 0) {
            volatile uint8_t *vc = (volatile uint8_t *)(uintptr_t)c->va;
            uint64_t cyc_diff = measure_pair_cycles(va, vc, iters);
            diff_sum += cyc_diff;
            diff_ok++;

            printf("trial %2d: SAME bank %2d rows %u/%u -> avg %.1f cyc\n",
                   t, bank_id, a->row, b->row, (double)cyc_same / (double)iters);
            printf("          DIFF bank %2d vs %2d rows %u/%u -> avg %.1f cyc\n",
                   bank_id, c->bank_id, a->row, c->row, (double)cyc_diff / (double)iters);
        } else {
            printf("trial %2d: SAME bank %2d rows %u/%u -> avg %.1f cyc\n",
                   t, bank_id, a->row, b->row, (double)cyc_same / (double)iters);
            printf("          DIFF: no suitable cross-bank same-context pair found\n");
        }
    }

    if (same_ok > 0) {
        printf("\nSAME-bank: %d pairs, mean avg %.1f cycles\n",
               same_ok, (double)same_sum / (double)same_ok / (double)iters);
    }
    if (diff_ok > 0) {
        printf("DIFF-bank: %d pairs, mean avg %.1f cycles\n",
               diff_ok, (double)diff_sum / (double)diff_ok / (double)iters);
    }

    if (same_ok > 0 && diff_ok > 0) {
        double same_avg = (double)same_sum / (double)same_ok / (double)iters;
        double diff_avg = (double)diff_sum / (double)diff_ok / (double)iters;
        printf("(ratio same/diff = %.2fx)\n", (diff_avg > 0.0) ? (same_avg / diff_avg) : 0.0);
        printf("=== END TIMING CHECK ===\n");
    }
}
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        } else if (!strcmp(argv[i], "--timing-trials") && i+1 < argc) {
            timing_trials = atoi(argv[++i]);
        } else { 
            usage(argv[0]);
            return 2;
        }
    ...
    if (timing_trials < 1) timing_trials = 1;
    if (timing_trials > 32) timing_trials = 32;
    ...
    if (do_timing) {
        run_timing_check(banks, timing_bank, timing_iters, timing_trials);
    }
