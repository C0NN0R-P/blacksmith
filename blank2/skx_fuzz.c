// skx_bank_sweep2.c with added simple Rowhammer fuzzing
...
static void usage(const char *argv0) {
    fprintf(stderr,
        ...
        " [--fuzz-bank N] [--fuzz-pairs N] [--fuzz-iters N] [--fuzz-pattern simple]\n"
        ...
        " --fuzz-pattern simple (double-sided; only option for now)\n",
        argv0);
}
...
// ---------- added fuzzing logic ----------
static void hammer_simple(uint64_t agg1_va, uint64_t victim_va, uint64_t agg2_va, uint64_t iters) {
    // double-sided hammer: access agg1 and agg2 alternately
    ...
    printf("hammering pair: agg1=0x%016" PRIx64 " victim=0x%016" PRIx64 " agg2=0x%016" PRIx64 " iters=%" PRIu64 "\n",
           agg1_va, victim_va, agg2_va, iters);
    uint8_t sink = 0;
    ...
    for (uint64_t i = 0; i < iters; i++) {
        ...
        if (i % progress_step == 0 && i > 0) {
            int percent = (int)((i * 100) / iters);
            int pos = (int)((i * bar_width) / iters);
            printf("\r[");
    ...
    printf("\r[==================================================] 100%%\n");
    printf("Hammering complete for this pair.\n");
    (void)sink;
}
static int check_flips(uint64_t va, uint64_t size) {
    // assume page-aligned, check for non-0xFF (initial pattern)
...
static void fuzz_bank(struct vec *bank, uint64_t fuzz_pairs, uint64_t fuzz_iters, void *buf, uint64_t bytes) {
    if (bank->n < 3) {
        warnx("fuzz: bank has <3 hits; skip (need agg1/victim/agg2)");
        return;
    }
    ...
    if (filtered.n < 3) {
        warnx("fuzz: filtered to %zu hits in common ctx; skip", filtered.n);
        free(filtered.v);
        return;
    }
    // sort by row for easy neighbor selection
    qsort(filtered.v, filtered.n, sizeof(struct hit), cmp_hit_row_col);
    // init memory with 0xFF pattern
...
static void fuzz_banks(struct vec banks[16], int fuzz_bank_id, uint64_t fuzz_pairs, uint64_t fuzz_iters, void *buf, uint64_t bytes, int best) {
    if (fuzz_bank_id < 0) fuzz_bank_id = best;
    if (fuzz_bank_id < 0 || fuzz_bank_id > 15) {
        warnx("fuzz: invalid bank %d", fuzz_bank_id);
        return;
    }
    fuzz_bank(&banks[fuzz_bank_id], fuzz_pairs, fuzz_iters, buf, bytes);
}
int main(int argc, char **argv) {
    ...
    const char *fuzz_pattern = "simple";
    for (int i = 1; i < argc; i++) {
        ...
        } else if (!strcmp(argv[i], "--fuzz-pattern") && i+1 < argc) {
            fuzz_pattern = argv[++i];
        } else {
            usage(argv[0]);
            return 2;
    ...
    if (fuzz_pairs > 0) {
        if (strcmp(fuzz_pattern, "simple")) warnx("fuzz: only 'simple' pattern supported");
        fuzz_banks(banks, fuzz_bank, fuzz_pairs, fuzz_iters, buf, bytes, best);
    }
    // cleanup
